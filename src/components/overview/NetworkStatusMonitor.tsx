...
const NetworkStatusMonitor: React.FC<NetworkStatusMonitorProps> = ({ 
  ...
  const connectionEvents = useMemo(() => {
    if (!networkStatus?.connectionHistory) return [];
    return networkStatus.connectionHistory.slice(-3).reverse();
  }, [networkStatus?.connectionHistory]);
...
export default memo(NetworkStatusMonitor);
